...
local function drawGvars()
  ...
  local gyroRate = getValue("s1")/1024*100   -- gyro rate (%)
  ...
  -- DM0: drift mode 1   => FL1 on (down, or right): Drag brake, FL2 on -> ABS
  -- DM1: drift mode 2   => FL1 on (down, or right): Traction control, FL2 not used
  local drag_brake = fm == 0 and getValue("s3") > 0
  local abs = fm == 0 and getValue("s4") > 0
  local traction_control = fm == 1 and getValue("s3") > 0


  lcd.drawText(3, LCD_H - 30 , "Gyro", SMLSIZE)
  lcd.drawNumber(5*FW + 2, LCD_H - 20 , gyroRate, MIDSIZE + RIGHT)
