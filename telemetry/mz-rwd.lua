...
local function drawGvars()
  ...
  local fm, _ = getFlightMode()
  -- DM0: default (grip/race mode) => FL1 on (down, or right): Drag brake, FL2 on -> ABS
  local drag_brake = fm == 0 and getValue("s3") > 0
  local abs = fm == 0 and getValue("s4") > 0

  if drag_brake then
    lcd.drawText(0, LCD_H - 22 , "DgB", SMLSIZE)
    lcd.drawNumber(5*FW + 2, LCD_H - 22 , dbRate, SMLSIZE + RIGHT + INVERS)
  else
    lcd.drawText(0, LCD_H - 22 , "DgB Off", SMLSIZE)
  end
  if abs then
    lcd.drawText(0, LCD_H - 15 , "ABS", SMLSIZE)
    drawCycleAndDuty(5*FW + 2, LCD_H - 15, absCycleT, absDutyC)
  else
    lcd.drawText(0, LCD_H - 15 , "ABS Off", SMLSIZE)
  end
